...
class Core extends PluginBase{
	...
	public function onLoad(){
		...
		$this->getLogger()->info(TF::colorize("&7Đang Tải Hộp Quà Đang Tự Sắp Xếp"));
	...
	public function onEnable(){
		...
		$this->getLogger()->info("Tất Cả Đã Được Sắp Xếp Và Chỉnh Sửa");
	}
	...
	public function onCommand(CommandSender $sender, Command $cmd, string $label, array $args) : bool{
		switch($args[0] ?? null){
			default: {
				$sender->sendMessage(TF::colorize($this->getConfig()->get("prefix")." &r&b/mysterybox &e[add | remove | set | create]"));
				break;
			}
			case "add": {
				if(isset($args[1]) == false){
					$sender->sendMessage(TF::colorize($this->getConfig()->get("prefix")." &r&b/mysterybox add &c[name] [box id] [amount]"));
					break;
				}
				if(isset($args[2]) == false){
					$sender->sendMessage(TF::colorize($this->getConfig()->get("prefix")." &r&b/mysterybox add &a[name] &c[box id] [amount]"));
					break;
				}
				if(isset($args[3]) == false or is_numeric($args[3]) == false or $args[3] < 1){
					$sender->sendMessage(TF::colorize($this->getConfig()->get("prefix")." &r&b/mysterybox add &a[name] [box id] &c[amount]"));
					break;
				}
				...
				if($box == null){
					$sender->sendMessage(TF::colorize($this->getConfig()->get("prefix")." &r&bHộp Quà Có ID &e{$id}&b Không Xác Định"));
					break;
				}
				
				$this->addMysteryKey($player, $id, $int);
				
				$sender->sendMessage(TF::colorize($this->getConfig()->get("prefix")." &r&bĐã Cộng &e{$id}&b Chìa Khoá × &b{$int}&e Tới &f{$args[1]}'s &bTài Khoản"));
				break;
			}
			case "remove": {
				if(isset($args[1]) == false){
				...
				if($box == null){
					$sender->sendMessage(TF::colorize($this->getConfig()->get("prefix")." &r&7Hộp Quà Có ID &e{$id}&7 Không Xác Định"));
					break;
				}
				...
				$sender->sendMessage(TF::colorize($this->getConfig()->get("prefix")." &r&bĐã Xoá &e{$id}&b Chìa Khoá × &b{$int}&e Đến &f{$args[1]}'s &7account"));
				break;
			}
			case "set": {
				if(isset($args[1]) == false){
					$sender->sendMessage(TF::colorize($this->getConfig()->get("prefix")." &r&b/mysterybox set &c[name] [box id] [amount]"));
					break;
				}
				if(isset($args[2]) == false){
					$sender->sendMessage(TF::colorize($this->getConfig()->get("prefix")." &r&b/mysterybox set &a[name] &c[box id] [amount]"));
					break;
				}
				if(isset($args[3]) == false or is_numeric($args[3]) == false or $args[3] < 1){
					$sender->sendMessage(TF::colorize($this->getConfig()->get("prefix")." &r&b/mysterybox set &a[name] [box id] &c[amount]"));
					break;
				}
				...
				$sender->sendMessage(TF::colorize($this->getConfig()->get("prefix")." &r&b Đã Trao &e{$id}&7 Chìa Khoá × &b{$int}&e Đến &f{$args[1]}'s &bTài Khoản"));
				break;
			}
			case "create": {
				if($sender instanceof Player == false){
					$sender->sendMessage(TF::colorize($this->getConfig()->get("prefix")." &r&bChỉ Hoạt Động Trong Game!"));
					break;
				}
				if(isset($args[1]) == false){
					$sender->sendMessage(TF::colorize($this->getConfig()->get("prefix")." &r&b/mysterybox create &c[box id]"));
					break;
				}
				if($this->getMysteryBox($args[1]) == null){
					$sender->sendMessage(TF::colorize($this->getConfig()->get("prefix")." &r&bKhông Nhận Dạng ID Hộp Quà"));
					break;
				}
				
				$this->getEventListener()->queueBoxCreation($sender, $args[1]);
				
				$sender->sendMessage(TF::colorize($this->getConfig()->get("prefix")." &r&bChỉ Định Block Làm Hộp Quà(Yêu Cầu Rương)"));
				break;
			}
			case "reload": {
				$this->onDisable();
				...
				$sender->sendMessage("&c[&bHộp Quà&c] &f>> &eHệ Thống Đã Được Tải Lại");
				break;
			}
		}
