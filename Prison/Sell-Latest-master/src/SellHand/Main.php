...
class Main extends PluginBase implements Listener{
	...
	public function onLoad(){
		$this->getLogger()->info("§eĐã Load Plugin - By MamChamPlu!");
	}
 ...
 Enabled Sell by Muqsit and JackMD.
	...
	public function onDisable(){
    	$this->getLogger()->info("§cPlugin Đã Tắt!");
  	}
	...
	public function onCommand(CommandSender $sender, Command $cmd, string $label, array $args): bool{
		switch(strtolower($cmd->getName())){
			...
			if(!($sender instanceof Player)){
				$sender->sendMessage(TF::RED . TF::BOLD ."Error: ". TF::RESET . TF::DARK_RED ."Dùng lệnh trong game mới thực thi!");
				return true;
				break;
			}
				...
				if($sender->hasPermission("sell") || $sender->hasPermission("sell.hand") || $sender->hasPermission("sell.all")){
					...
					if(!$sender->isSurvival()){
						$sender->sendMessage(TF::RED . TF::BOLD ."§2§lError: ". TF::RESET . TF::DARK_RED ."§cChỉ Xài Lệnh Khi Ở Chế Độ Sinh Tồn.");
						return false;
					}
					...
					if(isset($args[0]) && strtolower($args[0]) == "hand"){
						...
						if($this->sell->get($itemId) == null){
							$sender->sendMessage(TF::RED . TF::BOLD ."§2§lError: §r§cItem Hơi Lạ ". TF::RESET . TF::DARK_GREEN . $item->getName() . TF::DARK_RED ." §cKhong Xác Định.");
							return false;
						}
						...
						$sender->sendMessage(TF::GREEN . TF::GREEN . "§5$" . $price . " §dĐã Đã Tiền Vào Túi Của Bạn.");
						$sender->sendMessage(TF::GREEN . "§bĐã Làm" . TF::RED . "§3$" . $price . TF::GREEN . " §bAmount: §3" . $item->getCount() . " §bName: §3" . $item->getName() . " §bat §3$" . $this->sell->get($itemId) . " §beach.");
					...
					/* Sell All */
						...
						$sender->sendMessage(TF::RED . TF::RESET . TF::GRAY . "§aPLugin Này Chỉ Sử Dụng Cho Faction Prison Hoặc SkyBlock ......");
					}else{
						...
						$sender->sendMessage(TF::RED . "§5- " . TF::DARK_RED . "§b/sell info " . TF::GRAY . "- §7Đang Thực Hiện.....");
						return true;
					}
				}else{
