...
class Arena implements Listener {
    ...
    public function joinToArena(Player $player) {
        if(!$this->data["enabled"]) {
            $player->sendMessage("§c> Khu vực đang thiết lập!");
            return;
        }

        if(count($this->players) >= $this->data["slots"]) {
            $player->sendMessage("§c> Khu vực đã full!");
            return;
        }

        if($this->inGame($player)) {
            $player->sendMessage("§c> Bạn đã ở trong trò chơi!");
            return;
        }
        ...
        $this->broadcastMessage("§a> Người chơi {$player->getName()} đã vào ! §7[".count($this->players)."/{$this->data["slots"]}]");
    }
    ...
    public function disconnectPlayer(Player $player, string $quitMsg = "", bool $death = \false) {
        ...
        if(!$death) {
            $this->broadcastMessage("§a> Người chơi {$player->getName()} đã rời khỏi. §7[".count($this->players)."/{$this->data["slots"]}]");
        }
    ...
    public function startGame() {
        ...
        $this->broadcastMessage("Trò chơi bắt đầu!", self::MSG_TITLE);
		
		$player->addTitle("§aCố gắng lên nhé!");
    }
    ...
    public function startRestart() {
        ...
        $player->addTitle("§aBạn là người chiến thắng!");
        $this->plugin->getServer()->getPluginManager()->callEvent(new PlayerArenaWinEvent($this->plugin, $player, $this));
        $this->plugin->getServer()->broadcastMessage("§a[SkyWars] Người chơi {$player->getName()} đã chiến thắng ở khu vực {$this->level->getFolderName()}!");
        $this->phase = self::PHASE_RESTART;
    }
    ...
    public function onInteract(PlayerInteractEvent $event) {
        ...
        if($this->phase == self::PHASE_GAME) {
            $player->sendMessage("§c> Khu vực đang chơi");
			$player->sendMessage("§c> Hãy chọn khu khác hoặc đợi");
            return;
        }
        if($this->phase == self::PHASE_RESTART) {
            $player->sendMessage("§c> Khu vực đang khôi phục!");
			$player->sendMessage("§c-> dợi trong giây lát");
            return;
        }
    ...
    public function onLevelChange(EntityLevelChangeEvent $event) {
        ...
        if(!$player instanceof Player) return;
        if($this->inGame($player)) {
            $this->disconnectPlayer($player, "Bạn đã rời khỏi khu vực chơi!");
        }
    }
    ...
    public function loadArena(bool $restart = false) {
        if(!$this->data["enabled"]) {
            $this->plugin->getLogger()->error("Không thể tải được khu vực: Nó chưa được kích hoạt!");
            return;
        }
    ...
    public static function getChestItems(): array {
        $chestItems = [];
        $chestItems[0] = [
            256, 257, 258, 267, 268, 269, 270, 271, 272, 273, 275, 276, 277
        ];
        $chestItems[1] = [
            298, 299, 300, 301, 302, 303, 304, 305, 308, 309, 310, 311, 312, 313, 315, 316
        ];
        $chestItems[2] = [
