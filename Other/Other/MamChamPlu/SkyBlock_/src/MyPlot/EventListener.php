    ...
    public function onPlayerMove(PlayerMoveEvent $event) {
        ...
        if ($plot !== null and $plot !== $this->plugin->getPlotByPosition($event->getFrom())) {
            ...
            if ($plot->owner != "") {
				...
				$event->getPlayer()->addTitle("§l§bSkyBlock", "§a§lChào mừng bạn đã đến với đảo của§e ".$owner);
				$event->getServer()->broadcastMessage("§l§b[§aSKYBLOCK§b]§e ".$event->getPlayer()->getName()."§6 vừa đi sang đảo của §e ".$owner);
            } else {
