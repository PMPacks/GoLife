...
class EventListener implements Listener{
	...
	public function onInteract(PlayerInteractEvent $event) : void{
		...
		if($tile instanceof MysteryTile){
			...
			if($tile->in_use){
				$player->sendTip(TF::colorize($this->getCore()->getConfig()->get("prefix")." &r&7Hộp Quà Đang Được Sử Dụng!"));
				return;
			}
			...
			if($box->canOpen($player)){
				...
				$player->sendTip(TF::colorize($this->getCore()->getConfig()->get("prefix")." &r&7Bạn Không Có Chìa Khoá &e{$box->getName()}"));
			}
		}
	}
