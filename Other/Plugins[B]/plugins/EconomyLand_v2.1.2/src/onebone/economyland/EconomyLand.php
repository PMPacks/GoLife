	...
	public function onCommand(CommandSender $sender, Command $cmd, string $label, array $param) : bool{
		switch($cmd->getName()){
			...
			switch($sub){
				...
				if(trim($num) == ""){
					$sender->sendMessage("•§c Sử dụng :§6 /land move <id land>");
					return true;
				}
				if(!is_numeric($num)){
					$sender->sendMessage("•§c Sử dụng :§6 /land move <id land>");
				...
				case "invite":
					...
					if(trim($player) == "" or trim($landnum) == ""){
						$sender->sendMessage("•§c Suer dụng :§6 /land <invite> <id land> <người chơi>");
						return true;
					}
				...
				case "kick":
					...
					if(trim($player) === ""){
						$sender->sendMessage("•§c Sử dụng§6 /land kick <id land> <Người chơi");
						return true;
					}
					if(!is_numeric($landnum)){
						$sender->sendMessage($this->getMessage("land-num-must-numeric", array($landnum, "%2", "%3")));
				...
				case "invitee":
					...
					if(trim($landnum) == "" or !is_numeric($landnum)){
						$sender->sendMessage("•§c Sử dụng§6 /land invitee <id land>");
						return true;
					}
				...
				return true;
				default:
				$sender->sendMessage("•§c Sử dụng ".$cmd->getUsage());
			}
			return true;
			case "landsell":
			...
			default:
				...
				if(is_numeric($p)){
					...
					$sender->sendMessage("•§c Suer dụng§6 /landsell <id land>");
				}
			}
			return true;
		}
