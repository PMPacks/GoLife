...
class Main extends PluginBase implements Listener{
    public function onEnable() {
        ...
        Server::getInstance()->getLogger()->info("§aPlugin TimeRank Đã Hoạt Động!");
    ...
    public function onDisable() {
        Server::getInstance()->getLogger()->info("§cPlugin TimeRank Đã Tắt!");
		$this->config->save();
		$this->manager->save();
    }
		...
		if($date_at2[0] >= $date_ex2[0]){
			if($date_at2[1] >= $date_ex2[1]){
				if($date_at2[2] >= $date_ex2[2]){
					$this->getServer()->dispatchCommand(new ConsoleCommandSender(), "setgroup ".$player->getName() ." ".$this->manager->get("DefaultGroup"));
					$player->sendTip($this->manager->get("Vip_End_Message"));
					$this->config->remove($player->getName());
					$this->config->save();
					$this->getServer()->getLogger()->info("§8[§6BOT-GOE§8] §aRank của người chơi §6".$player->getName()." §ađã hết hạn!");
				}
			}
			}
		}
    ...
    public function onCommand(CommandSender $sender, Command $command, string $label, array $args):bool {
        switch($command->getName()){
            case "timegroup":
                ...
                if(!isset($args[2])){
                    $sender->sendMessage("§8[§6BOT-GOE§8] §cUse /timegroup add (player) (group) <Day>");
                    $sender->sendMessage("§8[§6BOT-GOE§8] §cUse /timegroup view");
                }
                if(strtolower($args[0]) == "add"){
                $target = $this->getServer()->getPlayer($args[1]);
                if($target === null){
                    $sender->sendMessage("§8[§6BOT-GOE§8] §cNgười chơi ".$args[1]." không tìm thấy trên máy chủ");
                    return true;
                }
                $date_at = date("d-m-Y");
                ...
                $this->getServer()->dispatchCommand(new ConsoleCommandSender(), "setgroup ".$target->getName() ." ".$args[2]);
				//$target->setDisplayName($this->tags->get($args[2]));
                $this->config->save();
                $sender->sendMessage("§a[§bBOT-GOE§a]§b Đã thêm thành công người chơi§d $args[1] §athành§d $args[2] §bTrong§d $args[3] §bNgày!");
