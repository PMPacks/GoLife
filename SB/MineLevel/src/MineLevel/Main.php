...
class Main extends PluginBase implements Listener{
    ...
    public function onEnable(){
		...
		$this->getLogger()->info("§eĐược Edit Sơ Sơ Bởi §b§lMamChamPLu");
    }
   ...
   //             $nchim = 1;
                $this->data->set(strtolower($p->getName()), $n);
                $this->data->save();
				$p->sendMessage("§b=-==-=§4F§eOS§aLevel§b=-==-=");
                $p->sendMessage("§b-=§aCấp Bạn Đạt: " . $this->getCurrentLevel($p). "§b=-");//xin config :v :v
				$this->getServer()->broadcastMessage("§b[§eGho§cst§6-§eLevel]§e Player §b" . $name . "§e Đạt Cấp: §b §e[§6" .$this->getCurrentLevel($p) . "§b]§l§e" .$name. "§2§l Cày Rất Chăm, Good Game");
                $this->getServer()->dispatchCommand(new ConsoleCommandSender(), "effect ".$name." 3 100 10");
				}
        }
        
        $p->setDisplayName("§b[§eLV:§a".$this->getCurrentLevel($p)."§b]§f ".$p->getName());
    }
