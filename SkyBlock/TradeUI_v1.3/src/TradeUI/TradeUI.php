...
class TradeUI extends PluginBase{
	...
	public function setEconomyProvider(EconomyProvider $provider){
        ...
        public function getBuyOrSellForm(Player $player) {
                ...
                $form->setTitle(RandomUtils::colorMessage("&e&k|&r&5&lChợ đêm&r&k&e|"));
                $content = "";
                $content .= RandomUtils::colorMessage("&l&c===========================\n");
                $content .= RandomUtils::colorMessage("&r§eChào, {$username}!\n\n");
                $content .= RandomUtils::colorMessage("&r§eNếu bạn muốn mua các mặt hàng từ\n");
                $content .= RandomUtils::colorMessage("&r§ecác người chơi khác, nhấp vào 'Mua'!\n\n");
                $content .= RandomUtils::colorMessage("&r&eNếu bạn muốn bán các mặt hàng của riêng bạn\n");
                $content .= RandomUtils::colorMessage("&r&eNhấp 'Bán'!\n");
                $content .= RandomUtils::colorMessage("&l&c===========================\n");
                $form->setContent($content);
                $form->setButton(RandomUtils::colorMessage("&e&k|&r&d&lMua&r&k&e|"));
                $form->setButton(RandomUtils::colorMessage("&e&k|&r&d&lBán&r&k&e|"));
                $form->send($player);
        }
        ...
        public function getChooseItemForm(Player $player) {
                ...
                $form->setTitle(RandomUtils::colorMessage("&e&k|&r&5&lChọn một vật phẩm để bán&r&k&e|"));
        ...
        public function getSellForm(Player $player, Item $item) {
                ...
                $form->setTitle(RandomUtils::colorMessage("&e&k|&r&5&lBán {$item->getName()}&r&k&e|"));
                $form->setInput(RandomUtils::colorMessage("&e&k|&r&d&lGiá&r&k&e|"), "Giá mà bạn bán vật phẩm này.");
                $form->setSlider(RandomUtils::colorMessage("&d&lSố lượng&r&e"), 1, $item->getCount(), 1, 1);
                $form->send($player);
        }
        ...
        public function getShopForm(Player $player, string $from = null) {
                ...
                if($from !== null){
                        ...
                        if(count($s) != 0){
                                ...
                                $form->setTitle(RandomUtils::colorMessage("&e&k|&r&5&l{$from}'s Chợ đen&r&k&e|"));
                                foreach($s as $r) {
                        ...
                        if($s !== null){
                                if(count($s) != 0){
                                        ...
                                        $page = $pg[1];
                                        $max = $pg[2];
                                        $form->setTitle(RandomUtils::colorMessage("&e&k|&r§d&lChợ đen&r&k&e|&r"));
                                        $form->setButton(RandomUtils::colorMessage("&l&k&e|&r&l&d← Quay lại&e&k|"));
                                        $form->setContent(RandomUtils::colorMessage(
                                            "            &l&dTổng cộng &7$items &dvật phẩm\n" .
                                            "          &l&e- &r&l&dTrang &7$page &dtrên &7$max&e -"
                                        ));
                                        $i = 1;
                                        ...
                                        foreach($s as $r){
                                                ...
                                                $form->setButton(RandomUtils::colorMessage(
                                                    "&l&k&e|&r&l&d{$r['trade']} &d(&8x{$r['amount']}&d)&e&k|&r\n" .
                                                    "&aNgười bán - &e{$r['username']}&r\n" .
                                                    "&8Price - &6{$r['price']}&r\n"
                                                ), "http://permat.comli.com/items/{$r['itemId']}-{$r['itemMeta']}.png");
                                        }
                                        $form->setButton(RandomUtils::colorMessage("&l&k&e|&r&l&dTrang tiếp theo →&e&k|"));
                                        $form->send($player);
                                }else{
        ...
        public function getMyOffersUI(Player $player){
                ...
                if(count($s) != 0){
                        ...
                        $form->setTitle(RandomUtils::colorMessage("&e&k|&r&5&lƯu Đãi Của Bạn&r&k&e|"));
                        $this->cache[$username]["ids"] = [];
                        foreach($s as $r){
                                $this->cache[$username]["ids"][] = $r['id'];
        ...
        public function getConfirmPurchaseForm(Player $player, int $id) {
                ...
                if(count($dat) > 0){
                        ...
                        $form->setTitle(RandomUtils::colorMessage("&e&k|&r&5&lBạn muốn mua item này?&r&k&e|"));
                        $content = "";
                        $content .= RandomUtils::colorMessage("&l&e===========================&r\n");
                        $content .= RandomUtils::colorMessage("&dNgười bán: &7" . $dat['username'] . "\n");
                        $content .= RandomUtils::colorMessage("&dTên vật phẩm: &7" . $dat['trade'] . " &r&5(" . Item::get($dat['itemId'])->getName() . ")\n");
                        $content .= RandomUtils::colorMessage("&dGiá: &7" . $dat['price'] . "\n");
                        $content .= RandomUtils::colorMessage("&dSố lượng: &7" . $dat['amount'] . "\n");
                        $content .= RandomUtils::colorMessage("&dEnchant: &7" . $this->formatEnchantmentIdsAsName($dat['enchants']) . "\n");
                        $content .= RandomUtils::colorMessage("&dMarket ID: &7" . $dat['id'] . "\n");
                        $content .= RandomUtils::colorMessage("&l&e===========================\n");
                        $form->setContent($content);
                        $form->setButton(RandomUtils::colorMessage("&a&lĐồng ý"));
                        $form->setButton(RandomUtils::colorMessage("&c&lKhông đồng ý"));
                        $form->send($player);
                }else{
        ...
        public function getConfirmDeleteForm(Player $player, int $id) {
                ...
                if(count($dat) > 0){
                        ...
                        $form->setTitle(RandomUtils::colorMessage("&e&k|&r&5&lBạn muốn xóa item này?&r&k&e|"));
                        $content = "";
                        $content .= RandomUtils::colorMessage("&l&e===========================&r\n");
                        $content .= RandomUtils::colorMessage("&4&lChú thích: &7Tất cả vật phẩm của bạn đã được trả lại.&r\n");
                        $content .= RandomUtils::colorMessage("&dTên vật phẩm: &7" . $dat['trade'] . " &r&5(" . Item::get($dat['itemId'])->getName() . ")\n");
                        $content .= RandomUtils::colorMessage("&dGiá: &7" . $dat['price'] . "\n");
                        $content .= RandomUtils::colorMessage("&dSố lượng: &7" . $dat['amount'] . "\n");
                        $content .= RandomUtils::colorMessage("&dEnchant: &7" . $this->formatEnchantmentIdsAsName($dat['enchants']) . "\n");
        ...
        public function handleFormResponse(Player $player, $formData, int $formId){
                ...
                switch($formId) {
                        ...
                        case $this->SELL_UI_ID: {
                                        ...
                                        //$this->getServer()->broadcastMessage(TextFormat::GREEN . "To view, type: " . TextFormat::RED . "/choden " . $username);
                                }
                                break;
