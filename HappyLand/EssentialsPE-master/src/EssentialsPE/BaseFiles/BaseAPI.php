...
use pocketmine\scheduler\AsyncPool as AsyncTask;
...
class BaseAPI{
    ...
    public function setAFKMode(Player $player, bool $state, bool $broadcast = true): bool{
        ...
        if(!$state && ($id = $this->getSession($player)->getAFKKickTaskID()) !== null){
            ...
            //$task = TaskScheduler::scheduleDelayedTask(new AFKKickTask($this, $player), $time * 20);
    ...
    public function scheduleAutoAFKSetter(): void{
        if(is_int($v = $this->getEssentialsPEPlugin()->getConfig()->getNested("afk.auto-set")) && $v > 0){
            //TaskScheduler::scheduleDelayedTask(new AFKSetterTask($this), 600); // Check every 30 seconds...
        }
    }
    ...
    public function getServerGeoLocation(): string{
        if($this->serverGeoLocation === null){
            //AsyncTask::submitTask(new GeoLocation(null));
        }
    ...
    public function createSession($player): array{
        ...
        //AsyncTask::submitTask(new GeoLocation($player));
    ...
    private function scheduleTPRequestTask(Player $player): void{
        //$task = TaskScheduler::scheduleDelayedTask(new TPRequestTask($this, $player), 20 * 60 * 5);
    ...
    private function cancelTPRequestTask(Player $player): void{
        //TaskScheduler::cancelTask($this->getSession($player)->getRequestToTaskID());
    ...
    public function fetchEssentialsPEUpdate(bool $install = false): bool{
        ...
        //AsyncTask::submitTask($task = new UpdateFetchTask($this->getUpdateBuild(), $install));
    ...
    public function scheduleUpdaterTask(): void{
        if($this->isUpdaterEnabled()){
            //TaskScheduler::scheduleDelayedTask(new AutoFetchCallerTask($this), $this->getUpdaterInterval() * 20);
        }
    }
