...
class KeyCommand extends PluginCommand{
	...
	public function __construct(string $name, Main $plugin){
		...
		$this->setDescription("§aĐưa chìa khóa cho người chơi.");
		...
		if($plugin instanceof Main){
			...
			if(isset($args[1])){
				...
				if(!$target instanceof Player){
					$sender->sendMessage(TextFormat::RED . "§aNgười chơi hông hợp lệ :v.");
					return true;
				}
			}else{
				if(!$target instanceof Player){
					$sender->sendMessage(TextFormat::RED . "§aGhi tên người muốn lấy key vào nè :v.");
					return true;
				}
			}
			if(!$plugin->getCrateType($args[0])){
				$sender->sendMessage(TextFormat::RED . "§aKhông Có Crate này :L.");
				return true;
			}
			...
			$sender->sendMessage(TextFormat::GREEN . ucfirst($args[0]) . "§a Chìa khóa đã được cho.");
			return true;
		}
		return true;
	}
