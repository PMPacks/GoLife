...
Class Main extends PluginBase implements Listener{
 ...
 public function onEnable(){
   ...
   if(!$sender instanceof Player){
	 ...
	 $sender->sendMessage("Đừng Có Giỡn Với Bố OK BABY!");
     return true;
     }
     switch($cmd->getName()){
        case "buyec":
     if (!isset($args[0]))		
		{
		$sender->sendMessage("§l§e→§aSử Dụng Lệnh: §e/buyec [§bID§e] [§4LEVEL§e] §anhé");
		$sender->sendMessage("§l§e→§eMỗi Cấp Là 10000 Xu Nha Mấy Bạn");
		$sender->sendMessage("§l§e→§cLưu Ý:§4 Nâng Cấp sẽ Không Có Cộng Dồn Đâu Nhé!");
			return true;
	 }
		...
		$sender->sendMessage("§aNào Nào! Hãy Kiếm Thêm Tiền Nhé Thì Mới Có Thể Nâng Cấp Vật Phẩm Bạn Lên Cấp Độ Cao");
         ...
         if($args[1] > $maxlevel){
		  ...
		  $sender->sendMessage("§l§6Bạn Chỉ §aENCHANT §a Tới Cấp Độ §e: ".$maxlevel);
			...
			$sender->sendMessage("§l§cĐể §eNắm Vững Tốt Về §eID §cHãy Ghi Lệnh /listec Để Lấy ID Chính Xác Nhất");
            }else{
...
$form->addLabel("§6Sử Dụng:§f/buyec [§eid§f] [§elevel§f] trong đó §6ID§f là mục enchant có sẵn bên dưới");
   $form->addLabel("§l§e
   §a0:§eBảo vệ
   ----------------
   §a1: §eBảo vệ khỏi lửa
