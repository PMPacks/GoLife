...
class SetVip extends PluginBase {
    ...
    public $pp;
    public $users;
    public $prefix = "§b§l•§a[§eSETVIP§a]§b• ";
            ...
            if ($s->hasPermission("setvip.cmd")) {
                if (count($args) == 3) {
                    ...
                    if ($p instanceof \pocketmine\Player) {
                        $p->sendMessage($this->prefix . " Bạn  nhận được vip: §e" . $rank . " trong " . $time . " Ngày");
                    }
                    settype($name, "string");
                    $s->sendMessage($this->prefix . "  set rank: §e" . $rank . "thành công cho §e" . $p->getName() . "trong" . $time . "ngày");
                    return true;
                }
            ...
            if (isset($args[0])) {
                if ($s->hasPermission("setvip.cmd")) {
                    if ($this->time->exists($args[0])) {
                        ...
                        $s->sendMessage($this->prefix . "Xoá vip thành công của người chơi: " . $args[0]);
                    } else {
                        $s->sendMessage($this->prefix . "Người chơi: " . $args[0] . " hiện tại chẳng có vip");
                        return true;
                    }
                } else {
                    $s->sendMessage($this->prefix . "Cách dùng: /removevip <tên>");
                    return true;
                }
            } else {
                $s->sendMessage($this->prefix . "Bạn không có quyền dùng lẹnh này!");
                return true;
            }
        }
    }
            ...
            foreach ($all as $name => $stuff) {
                ...
                if ($i[1] < 1) {
                    ...
                    if ($p !== null && $this->getServer()->getPlayer($name) !== null) {
                        $p->sendMessage("§7[§eVIP§7]§6 VIP " . $i[2] . "Của bạn đã hết hạn , nếu muốn tiếp tục có vip xin hãy nạp thêm!");
                    }
                    ...
                    if (isset($pcfg["holding-vip"])) {
                        if (count($pcfg["holding-vip"]) >= 1) {
                            ...
                            if ($group == null) {
                                if ($p !== null && $this->getServer()->getPlayer($name) !== null) {
                                    $p->sendMessage($this->prefix . " " . $is[1] . "Galaxy-Craft");
                                }
