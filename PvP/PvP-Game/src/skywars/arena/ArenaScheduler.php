        ...
        switch ($this->plugin->phase) {
            case Arena::PHASE_LOBBY:
                if(count($this->plugin->players) >= 2) {
                    $this->plugin->broadcastMessage("§c[§bGOE-§ePvP§c]-§b Bắt Đầu Trong" . Time::calculateTime($this->startTime) . " Giây.", Arena::MSG_TIP);
                    $this->startTime--;
                    if($this->startTime == 0) {
                    ...
                    $this->plugin->broadcastMessage("§c[§bGOE-§ePvP§c]-§b Bạn Phải Đợi Thêm Người Để Bắt Đầu Trận Chiến!", Arena::MSG_TIP);
                    $this->startTime = 40;
                }
                break;
            case Arena::PHASE_GAME:
                $this->plugin->broadcastMessage("§c[§bGOE-§ePvP§c]-§b Hiên Đang Có " . count($this->plugin->players) . " Người Đang Đấu, Thời Gian Kết Thúc: " . Time::calculateTime($this->gameTime) . "", Arena::MSG_TIP);
                switch ($this->gameTime) {
                    case 15 * 60:
                        $this->plugin->broadcastMessage("§c[§bGOE-§ePvP§c]-§b Làm Đầy Rương Trong 5 Phút.");
                        break;
                    case 11 * 60:
                        $this->plugin->broadcastMessage("§c[§bGOE-§ePvP§c]-§b Làm Đầy Rương Trong 1 Phút.");
                        break;
                    case 10 * 60:
                        $this->plugin->broadcastMessage("§c[§bGOE-§ePvP§c]-§b Đã Làm Đầy Rương.");
                        break;
                }
            ...
            case Arena::PHASE_RESTART:
                $this->plugin->broadcastMessage("§a> Về Nhà Sau {$this->restartTime} Giây.", Arena::MSG_TIP);
                $this->restartTime--;
        ...
        switch ($this->plugin->phase) {
            case Arena::PHASE_LOBBY:
                if(count($this->plugin->players) >= $this->plugin->data["slots"]) {
                    $signText[2] = "§l§0§6Đầy";
                    $signText[3] = "§l§0§bMap: §7{$this->plugin->level->getFolderName()}";
                }
                else {
                    $signText[2] = "§l§0§aTap To join";
                    $signText[3] = "§l§0§bMap: §7{$this->plugin->level->getFolderName()}";
                }
                break;
            case Arena::PHASE_GAME:
                $signText[2] = "§l§0§5Đang Chơi";
                $signText[3] = "§l§o§bMap: §7{$this->plugin->level->getFolderName()}";
                break;
            case Arena::PHASE_RESTART:
                $signText[2] = "§l§o§cĐang Tạo Lại";
                $signText[3] = "§l§o§8Map: §7{$this->plugin->level->getFolderName()}";
                break;
        }
