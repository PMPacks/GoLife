...
class Arena implements Listener {
    ...
    public function joinToArena(Player $player) {
        ...
        $this->broadcastMessage("§c[§bGOE-§ePvP§c]-§b Người Chơi§c {$player->getName()} §bĐã Tham Chiến! §a[".count($this->players)."/{$this->data["slots"]}]");
    }
    ...
    public function disconnectPlayer(Player $player, string $quitMsg = "", bool $death = \false) {
        ...
        if(!$death) {
            $this->broadcastMessage("§c[§bGOE-§ePvP§c]-§b Người Chơi {$player->getName()} Đã Bỏ Cuộc. §a[".count($this->players)."/{$this->data["slots"]}]");
        }
    ...
    public function startGame() {
        ...
        $this->broadcastMessage("Cuộc Chiến Bắt Đầu!", self::MSG_TITLE);
    }
    ...
    public function startRestart() {
        ...
        $player->addTitle("§bBạn Đã Thắng!");
        $this->plugin->getServer()->getPluginManager()->callEvent(new PlayerArenaWinEvent($this->plugin, $player, $this));
        $this->plugin->getServer()->broadcastMessage("§c[§bGOE-§ePvP§c]-§b Người Chơi {$player->getName()} Đã Thắng Game Tại {$this->level->getFolderName()}!");
        $this->phase = self::PHASE_RESTART;
    }
    ...
    public function onInteract(PlayerInteractEvent $event) {
        ...
        if($this->phase == self::PHASE_GAME) {
            $player->sendMessage("§c[§bGOE-§ePvP§c]-§b Đang Đấu");
            return;
        }
        if($this->phase == self::PHASE_RESTART) {
            $player->sendMessage("§c[§bGOE-§ePvP§c]-§b Đag Làm Lại Chiến Trường!");
            return;
        }
    ...
    public function onDeath(PlayerDeathEvent $event) {
        ...
        $this->broadcastMessage("§c[§bGOE-§ePvP§c]-§b {$this->plugin->getServer()->getLanguage()->translate($event->getDeathMessage())} §7[".count($this->players)."/{$this->data["slots"]}]");
    ...
    public function onLevelChange(EntityLevelChangeEvent $event) {
        ...
        if(!$player instanceof Player) return;
        if($this->inGame($player)) {
            $this->disconnectPlayer($player, "§c[§bGOE-§ePvP§c]-§b Bạn Được Đưa Về An Toàn!");
        }
    }
    ...
    public static function getChestItems(): array {
        $chestItems = [];
        $chestItems[0] = [
            256, 257, 258, 267, 268, 269, 270, 271, 272, 273, 274, 275, 276, 277, 278, 279, 261
        ];
        $chestItems[1] = [
            298, 299, 300, 301, 302, 303, 304, 305, 306, 307, 308, 309, 310, 311, 312, 313, 314, 315, 316, 317, 400
        ];
        $chestItems[2] = [
            319, 320, 297, 391, 392, 393, 396, 400, 411, 412, 423, 424, 261
        ];
        $chestItems[3] = [
            1, 2, 3, 4, 5, 12, 13, 14, 15, 16, 17, 18, 82, 35, 45, 262
        ];
        $chestItems[4] = [
            263, 264, 265, 266, 280, 297, 322, 262
        ];
        return $chestItems;
    }
